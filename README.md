...
- **页面 2 · 趋势分析**（`/trend`）：使用 ECharts 图表展示近十二个月交换记录的月度趋势，支持折线图/柱状图两种展示方式切换，包含「交换总数」与「已完成数」两组数据对比；页面底部附带周期汇总统计卡片，展示近十二月交换总数、已完成数及整体完成率
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/statistics` | 统计概览（总记录数、已完成数、进行中数、最近进行中记录） |
| GET | `/api/statistics/monthly` | 按月统计趋势，返回近十二个月的交换数据，按月份升序排列，返回 `{ items: [{ month: "YYYY-MM", total_count: number, completed_count: number }, ...] }`；以寄出日期 `sent_date` 归属月份，无数据的月份返回 0 |
