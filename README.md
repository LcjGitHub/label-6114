...
- **页面 4 · 新增/编辑交换记录**（`/new`、`/edit/:id`）：`n-form` 表单，字段包括书名、对方昵称、寄出日期、收到日期、是否完成、备注说明（多行文本，选填，最长 500 字）；填写收到日期时自动将是否完成开关设为开启，清空收到日期时若此前因填写日期而自动完成则恢复为未完成，用户手动操作开关后不再受日期联动影响
