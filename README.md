...
- **页面 2 · 交换记录列表**（`/list`）：`n-data-table` 展示全部记录，状态列用 `n-tag` 区分「已完成 / 进行中」，支持编辑、删除
- **页面 3 · 新增/编辑**（`/new`、`/edit/:id`）：`n-form` 表单，字段包括书名、对方昵称、寄出日期、收到日期、是否完成

> 顶部导航栏可在三个页面之间自由切换，「数据概览」为默认首页。
